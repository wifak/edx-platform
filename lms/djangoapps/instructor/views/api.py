...
def get_students_who_may_enroll(request, course_id):
    ...
    try:
        ...
        success_status = _("Your list of invited students who have not enrolled yet is being generated! You can view the status of the generation task in the 'Pending Instructor Tasks' section.")
        return JsonResponse({"status": success_status})
    except AlreadyRunningError:
        already_running_status = _("A task for generating a list of invited students who have not enrolled yet is already in progress. Check the 'Pending Instructor Tasks' table for the status of the task. When completed, the report will be available for download in the table below.")
        return JsonResponse({"status": already_running_status})
